                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="mb-6">
                        ...
                        <h1 class="text-center text-2xl font-bold mb-6 pb-2 border-b-2 border-orange-400 inline-block">
                            Modifica Pane: {{ $bread->name }}
                        </h1>
                        
                        <form action="{{ route('breads.update', $bread->id) }}" method="POST" class="space-y-6">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text">
                                <div>
                                    ...
                                    <input type="text" name="name" id="name"
                                        class="w-full border border-gray-300 dark:border-gray-600 rounded-md py-2 px-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:text-white @error('name') border-red-500 @enderror" 
                                        required value="{{ old('name', $bread->name) }}">
                                    @error('name')
                                ...
                                <div>
                                    ...
                                    <select name="suppliers[]" id="suppliers" multiple
                                        ...
                                        @foreach($suppliers as $supplier)
                                            <option value="{{ $supplier->id }}" {{ $bread->suppliers->contains($supplier->id) ? 'selected' : '' }}>
                                                {{ $supplier->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <p class="text-xs text-gray-500 mt-1">Tenere premuto CTRL (o CMD su Mac) per selezionare più fornitori</p>
                                    @error('suppliers')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
