            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="mb-6">
                        ...
                        <form action="{{route('points.update', $point->id)}}" method="POST" class="space-y-6">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="name" id="name" value="{{$point->name}}"
                                    ...
                                    <select name="suppliers[]" id="suppliers" multiple
                                        ...
                                        @foreach($suppliers as $supplier)
                                            <option value="{{ $supplier->id }}" 
                                                {{ in_array($supplier->id, $point->suppliers->pluck('id')->toArray()) ? 'selected' : '' }}>
                                                {{ $supplier->name }}
                                            </option>
                                        @endforeach
                                    </select>
